...
SELECT productLine, COUNT(DISTINCT productName) AS num_products
FROM products 
GROUP BY productLine;

-- 2) Per ogni linea prodotto, contare il numero totale di prodotti presenti:
SELECT productLine, COUNT(productName) AS num_products
FROM products 
GROUP BY productLine;
...
SELECT customerName, YEAR(paymentDate) AS year, SUM(amount) AS total_paid
FROM customers INNER JOIN payments ON customers.customerNumber = payments.customerNumber
GROUP BY customerName, year;

-- 5) Per i prodotti che sono stati ordinati almeno 10 volte, trovare il numero di ordini in cui compaiono. Ordinare l'output dal più ordinato al meno ordinato. (non importa la quantità ordinata)
SELECT productName, COUNT(DISTINCT orderNumber) AS num_orders
FROM products INNER JOIN orderdetails ON products.productCode = orderdetails.productCode
GROUP BY productName HAVING num_orders >= 10
...
SELECT SUM(quantityOrdered) AS total_pieces
FROM orderdetails;
...
-- 10) Trovare codice, linea prodotto, descrizione e quantità in magazzino dei prodotti che tra le caratteristiche presentano “steering function” (suggerimento: cercare nel campo descrizione)
SELECT productCode, productLine, productDescription, quantityInStock
FROM products
WHERE productDescription LIKE '%steering function%';

-- 11) Stampare check number e importo dei pagamenti effettuati nel secondo semestre del 2004, se maggiori di 15000 dollari
SELECT checkNumber, amount
FROM payments
WHERE YEAR(paymentDate) = 2004 AND MONTH(paymentDate) >= 7 AND amount > 15000;
...
SELECT firstName, lastName, COUNT(DISTINCT customerNumber) AS num_customers FROM employees
